...
### 2.4 中優先: LLM 解析時間が累積 timeout に入っていない [完了済み]

#### 対応状況

- 完了済み
- `evaluation_duration_sec` / `total_evaluation_duration_sec` を追加済み
- `loop_duration_sec` / `total_loop_duration_sec` を導入済み
- `ExperimentResult.metrics` / state / execution_log で実行時間と評価時間を分離して観測可能
...
### 2.5 中優先: LLM 解析の失敗が観測しづらい [完了済み]

#### 対応状況

- 完了済み
- `execution_log` に `llm_analysis_used` を記録済み
- 失敗フォールバック時は `llm_analysis_fallback_reason` を記録済み
- state にも `llm_analysis_used` / `llm_analysis_fallback_reason` を保持し、CLIログから追跡可能
