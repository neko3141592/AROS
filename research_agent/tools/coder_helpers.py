...
from tools.task_context import summarize_failure_for_prompt
...
def _maybe_extract_legacy_files_payload(raw_text: str) -> dict[str, str] | None:
    """
    旧式の {"files": {...}} / {path: content} 形式を互換的に受け取る。
    """
...
def _build_failure_context(
    ...
    failure = summarize_failure_for_prompt(evaluator_feedback, execution_stderr)
    feedback = evaluator_feedback or {}
    summary = failure["summary"] or "unknown_failure"
    ...
    stderr_text = failure["stderr"] or "(empty)"

    return (
        f"summary: {summary}\n"
        "# Previous Failure Context\n"
        "Prioritize fixing the direct cause of the previous failure and avoid unrelated changes.\n"
        f"On retries, inspect {entrypoint} before editing it if that file exists.\n"
...
def _build_tool_result_message(tool_result: Mapping[str, Any]) -> str:
    """
    ツール実行結果を tool message 用の文字列へ整形する。
    """
...
def _build_workspace_state_payload(
    ...
    Args:
        ...
        entrypoint: Evaluator が実行する相対エントリーポイント。
    """
    workspace_files = _load_workspace_files(run_paths, list_files_fn, read_file_fn)
    if not workspace_files:
        raise ValueError("workspace must contain at least one file before evaluator step.")
    if entrypoint not in workspace_files:
        raise ValueError(
            f"workspace must contain the execution entrypoint before evaluator step: {entrypoint}"
        )

    entrypoint_code = workspace_files[entrypoint]
    save_generated_code_fn(run_paths, entrypoint_code, filename=entrypoint)
    return {
        "generated_code": entrypoint_code,
